...
Краткое обоснование решений под тестовое задание (Tool Use, реальные API, edge cases).

**FastAPI + HTTP API**  
Явный контракт для сдачи: валидация запроса, OpenAPI, `422` при нехватке данных. Проще демонстрировать и тестировать, чем только CLI.

**LangGraph, а не CrewAI / multi-agent**  
Нужны линейные стадии (извлечь → собрать → отчёт) и одна ветка `insufficient_data`. Два tools не оправдывают нескольких «агентов»: выше latency, сложнее логи, те же требования ТЗ закрываются одним графом.

**Deterministic tool calling (без ReAct)**  
ТЗ требует осознанный вызов tools и правильные аргументы (`Apple`, не весь промпт). Фиксированный вызов обоих tools в `collect_data` предсказуемее, чем если LLM сам решает, вызывать ли API — меньше пропусков tools и галлюцинаций аргументов.
...
БД, кэш, очереди, agentic tool loop, полный multi-agent — вне scope MVP и ТЗ на 2–4 часа.
